...
void calcGST(const Mat& inputImg, Mat& imgCoherencyOut, Mat& imgOrientationOut, int W)
{
	...
	//Mat img_test(2, 2, CV_64F, Scalar(0));
	//img_test.at<double>(0, 0) = imgDiffXXsmooth.at<double>(W, W);
	//img_test.at<double>(1, 1) = imgDiffYYsmooth.at<double>(W, W);
	//img_test.at<double>(0, 1) = imgDiffXYsmooth.at<double>(W, W);
	//img_test.at<double>(1, 0) = img_test.at<double>(0, 1);
	//Mat Mat_e, Mat_v;
	...
	imgCoherencyOut = imgCoherency.clone();
	imgOrientationOut = imgPhase.clone();
}
...
int main()
{
	...
	while (true)
	{
		...
		//bitwise_and(imgCoherencyBin, imgOrientationBin, imgBin);
